...
Planet::Planet(int *complexity) : _complexity(complexity)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        _planetArray[i]->setPositionY(-1000);
    }
}
    ...
    std::thread spawnPlanetThread([&] {
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (_planetArray[randomIndex]->getPositionY()
                        + _planetArray[randomIndex]->getTextureRect().getMaxY()
                              * _planetArray[randomIndex]->getScale()
                    <= 0)
                    break;
            }
            _planetArray[randomIndex]->setScale(cocos2d::random<double>(0.5, 2));
            _planetArray[randomIndex]->setOpacity(cocos2d::random(200, 256));
            _planetArray[randomIndex]->setPosition(
                cocos2d::Vec2(cocos2d::random<int>(0, sizeScene.x
                                                          - _planetArray[randomIndex]
                                                                ->getTextureRect()
                                                                .getMaxX()),
                              sizeScene.y));
            _layer->addChild(_planetArray[randomIndex]);
            _planetArray[randomIndex]->runAction(cocos2d::Sequence::create(
                cocos2d::MoveTo::create(
                    6 + cocos2d::random(0, 4) - (*_complexity) * 0.1,
                    cocos2d::Vec2(_planetArray[randomIndex]->getPositionX(),
                                  -_planetArray[randomIndex]->getTextureRect().getMaxY()
                                      * _planetArray[randomIndex]->getScale())),
                cocos2d::CallFunc::create([this, randomIndex]() {
                _planetArray[randomIndex]->retain();
                    _layer->removeChild(_planetArray[randomIndex]);
                }),
                nullptr));
